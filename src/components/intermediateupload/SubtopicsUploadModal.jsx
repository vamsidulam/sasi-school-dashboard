...
import { extractSubtopicsFromFile } from './extractIntermediateFile.js'
import { intUploadApi, intStreamsApi, intSubjectsApi, intTopicsApi } from '../../lib/intermediateApi.js'
import { useAuth } from '../../hooks/useAuth.js'

const COLUMN_FIELDS = [
  { key: 'name', label: 'Subtopic Name', required: true, placeholder: 'e.g. A or Subtopic/Name' },
  { key: 'topic', label: 'Topic Name', required: true, placeholder: 'e.g. B or Topic' },
]
...
export default function SubtopicsUploadModal({ open, onClose, onUploaded }) {
  const { user } = useAuth()
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { rows, warnings: extractWarnings } = await extractSubtopicsFromFile(file, extractOptions)

      if (!rows.length) {
        throw new Error('No usable rows found. Check column mappings.')
      }

      const response = await intUploadApi.subtopics({
        fileName: file.name,
        rows,
    ...
    >
      ...
      >
        <header className="flex items-center justify-between border-b border-gray-200 px-5 py-3">
          <h2 className="text-base font-semibold text-gray-900">Upload Subtopics</h2>
          <button
            type="button"
            onClick={onClose}
            disabled={submitting}
        ...
        <div className="flex-1 space-y-5 overflow-y-auto px-5 py-4">
          ...
          <section className="rounded-md border border-blue-200 bg-blue-50 p-3">
            ...
            <p className="mb-2 text-[11px] text-gray-600">
              Select the stream and subject for these subtopics. The topic name should be provided in the file. This ensures subtopics are mapped to the correct parent topic within the selected subject.
            </p>
          ...
          <section className="rounded-md border border-gray-200 bg-gray-50 p-3">
            ...
            <p className="mt-2 text-[11px] text-gray-500">
              Upload Excel (.xlsx, .xls) or CSV file with subtopic names.
            </p>
          </section>
          ...
          <section className="rounded-md border border-amber-200 bg-amber-50/40 p-3">
            ...
            <p className="mb-2 text-[11px] text-gray-600">
              <strong>Column mapping:</strong> Enter column letter or header name. Topic name will be looked up in database.
            </p>
          ...
          {result ? (
            <div className="space-y-2 rounded-md border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-800">
              <div className="font-medium">
                Inserted: {result.inserted ?? 0} subtopics under "{result.topicName}"
              </div>
