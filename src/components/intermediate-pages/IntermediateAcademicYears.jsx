...
import AcademicYearFormModal from './academicyears/AcademicYearFormModal.jsx'
import { intAcademicYearsApi } from '../../lib/intermediateApi.js'

export default function IntermediateAcademicYears() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await intAcademicYearsApi.listAll()
      setItems(res.items || [])
    } catch (err) {
      setError(err)
    } finally {
  ...
  const handleSubmit = async (values) => {
    if (formMode === 'edit' && editing) {
      const updated = await intAcademicYearsApi.update(editing.id, values)
      setItems((prev) => prev.map((p) => (p.id === updated.id ? updated : p)))
    } else {
      const created = await intAcademicYearsApi.create(values)
  ...
  const handleConfirmDelete = async () => {
    if (!deleting) return
    await intAcademicYearsApi.remove(deleting.id)
