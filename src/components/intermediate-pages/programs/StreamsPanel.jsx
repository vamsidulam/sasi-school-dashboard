...
import StreamFormModal from './StreamFormModal.jsx'
import { intStreamsApi } from '../../../lib/intermediateApi.js'
...
export default function StreamsPanel() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await intStreamsApi.listAll()
      setItems(res.items || [])
    } catch (err) {
      setError(err)
    } finally {
  ...
  const handleSubmit = async (values) => {
    if (formMode === 'edit' && editing) {
      const updated = await intStreamsApi.update(editing.id, values)
      setItems((prev) => prev.map((p) => (p.id === updated.id ? updated : p)))
    } else {
      const created = await intStreamsApi.create(values)
  ...
  const handleConfirmDelete = async () => {
    if (!deleting) return
    await intStreamsApi.remove(deleting.id)
    ...
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-4">
        <p className="text-sm text-gray-500">
          Streams are global — they apply across all branches. Name must be unique.
        </p>
        <button
          type="button"
          onClick={() => {
