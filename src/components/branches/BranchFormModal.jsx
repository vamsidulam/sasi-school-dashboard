...
export default function BranchFormModal({ open, mode = 'create', initial = null, onClose, onSubmit }) {
  ...
  useEffect(() => {
    if (open) {
      setForm(initial ? { name: initial.name, code: initial.code } : emptyForm())
      setError(null)
      setSubmitting(false)
    }
  }, [open, initial])
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true)
    try {
      await onSubmit?.({ name: form.name.trim(), code: form.code.trim() })
    } catch (err) {
    ...
    >
      ...
      >
        <header className="flex items-center justify-between border-b border-gray-200 px-5 py-3">
          <h2 id="branch-modal-title" className="text-base font-semibold text-gray-900">
        ...
        <footer className="flex items-center justify-end gap-2 border-t border-gray-200 bg-gray-50 px-5 py-3">
          ...
          >
            ...
            {mode === 'edit' ? 'Save changes' : 'Create branch'}
          </button>
        </footer>
      </form>
    </div>
